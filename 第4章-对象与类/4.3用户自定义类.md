...
![返回引用](https://cdn.jsdelivr.net/gh/RuiqiVan/core_java_I_img/res/unit4/return_reference.png)

# 4.3.9 基于类的访问权限
Employee类的方法可以访问任何Employee类的私有字段.(这在逻辑上也是理所当然的，不然我们如何比较两个都是同一种类的对象是否相等呢)
```` java
class Employee{
    ...
    public boolean equals(Employee other){
        return name.equals(other.name);//通过比较other的name看是否相等
    }
}
````


# 4.3.10私有方法 && 4.3.11final实例字段
如要将公有方法改为私有方法,将方法前的public改为private即可   

可以将实例字段设置为final,不过必须在构造对象时初始化    
如为基本类型(如int,boolean即不为类类型),则在初始化后值无法改变    

final标记对象变量后,对象变量将不能在引用别的对象,只能指示当前对象,不过当前对象如果有修改器方法,仍可以更改自身    
如 private final StringBuilder eva;  
在构造器中可以初始化为eva=new StringBuilder();  
但之后再eva=new StringBuilder();就是错的  
但仍可以更改自身eva.append("good");
